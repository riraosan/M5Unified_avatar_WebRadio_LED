    ...
    while (self->_task_running)
    {
      ...
      if (!flg_i2s_started)
      {
        ...
        if (self->_cfg.use_dac && self->_cfg.dac_zero_level != 0)
        {
          ...
          size_t write_bytes;
          if (self->_cfg.bits_per_sample == I2S_BITS_PER_SAMPLE_16BIT) {
            // standard logic with 16 bits
            if (i2s_write(i2s_port, (void*)sound_buf, dma_buf_len * sizeof(int16_t), &write_bytes, portMAX_DELAY) != ESP_OK) {
              log_e("i2s_write has failed");
            }
          } else {
            if (self->_cfg.bits_per_sample > 16) {
              // expand e.g to 32 bit for DACs which do not support 16 bits
              if (i2s_write_expand(i2s_port, (void*)sound_buf, dma_buf_len * sizeof(int16_t), I2S_BITS_PER_SAMPLE_16BIT, I2S_BITS_PER_SAMPLE_32BIT, &write_bytes, portMAX_DELAY) != ESP_OK) {
                log_e("i2s_write_expand has failed");
              }
            } else {
              log_e("invalid bits_per_sample: %d", I2S_BITS_PER_SAMPLE_32BIT);
            }
          }
          memset(float_buf, 0, dma_buf_len * sizeof(float));
        }
...
/// (I2Sをモノラル設定にした場合は同じデータが２チャンネル分送信されてしまうため、敢えてステレオ扱いとしている);
      ...
      if (self->_cfg.bits_per_sample == I2S_BITS_PER_SAMPLE_16BIT) {
        // standard logic with 16 bits
        if (i2s_write(i2s_port, (void*)sound_buf, dma_buf_len * sizeof(int16_t) << self->_cfg.buzzer, &write_bytes, portMAX_DELAY) != ESP_OK) {
          log_e("i2s_write has failed");
        }
      } else {
        if (self->_cfg.bits_per_sample > 16) {
          // expand e.g to 32 bit for DACs which do not support 16 bits
          if (i2s_write_expand(i2s_port, (void*)sound_buf, dma_buf_len * sizeof(int16_t) << self->_cfg.buzzer, I2S_BITS_PER_SAMPLE_16BIT, self->_cfg.bits_per_sample, &write_bytes, portMAX_DELAY) != ESP_OK) {
            log_e("i2s_write has failed");
          }
        } else {
          log_e("invalid bits_per_sample: %d", I2S_BITS_PER_SAMPLE_32BIT);
